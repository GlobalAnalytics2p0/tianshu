...
- Closed-loop supervision is part of the publishing flow. Use `node scripts/supervise-ai-flow.mjs --phase preflight --write-ledger` before writing, `node scripts/supervise-ai-flow.mjs --phase post-generation --write-ledger` after chapter/manifest/note updates, and `node scripts/supervise-ai-flow.mjs --phase post-publish` after push. Its decision values are gates: `ready-for-generation`, `ready-to-publish`, and `published-and-verified` allow the next step; `blocked` must be fixed or reported. Do not write the ledger during post-publish verification after push, or the verification step itself will leave a dirty repository.
- The supervisor writes the latest machine-readable report to `tmp/automation-supervision/latest.json` and durable improvement notes to `src/resource/automation-supervision-log.md`. Repeated warnings, especially AI-even pacing, weak first-window turns, weak final hooks, or cross-title overlap, should be converted into concrete next-run revision targets rather than ignored because they are non-blocking.
- Supervisor warnings must be attributed to the chapter that caused them. Old locked-chapter warnings may remain visible in the full-series audit, but they must not be summarized as「本輪新章」problems. Current-batch lessons are derived only from the chapter references audited for the active batch.
...
- An automation-owned partial batch is distinct from deferred publish debt. When `active-batch.json` validates against the same baseline HEAD and only expected chapter/note paths are dirty, preflight should return `resume-generation` and list exactly which titles remain. If all five are complete but uncommitted, it should return `resume-validation`. Any unexpected chapter number, title path, HEAD change, or note/manifest mismatch invalidates ownership and blocks automatic continuation.
...
- The paired reader must leave a three-layer review receipt for each new chapter: (1) `AI 感` cites concrete scene rhythm, bodily friction, ordinary work, and non-template transitions; (2) `理論一致性` cites the title's `風格規則.md`, `核心靈魂檔案.md`, `作者思路.md`, or `伏筆事件台帳.md` and states why the new rule does not contradict them; (3) `前後邏輯` names the previous chapter and traces at least one hook, injury, debt, relationship, object, or time state into the new chapter. A `反證` line must say what future discovery would force the chapter back to revision. Only then may the receipt say `判定：通過`.
...
- For automation publishes, `node scripts/verify-site-publication.mjs` passes after push. If it does not pass, the run may be locally complete or pushed, but it must not be described as live on the website.
  - The next generation preflight must also require `node scripts/ensure-pages-deployment.mjs --retry` and live publication verification to pass. A Git-synchronized commit with a failed, queued, or unverified Pages deployment is publish debt and blocks new chapter generation.
  - Pages deploy artifacts must contain only the reader runtime contract: `manifest.json`, manifest-referenced chapter text, and manifest-referenced cover/hero images. Author notes, QA frames, audio, video, and short-video production files remain in the repository but must never be copied to `dist`.
