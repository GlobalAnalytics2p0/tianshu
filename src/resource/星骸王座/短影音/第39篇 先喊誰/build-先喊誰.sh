...
from pathlib import Path
base=Path(os.environ['BASE']); safe=float(os.environ['SUBTITLE_CUTOFF']); ffprobe=os.environ['FFPROBE']; voice=base/'voice'/'edge-acted-narration.m4a'
...
if dur > safe - .2:
    raise SystemExit(f'voice track {dur:.3f}s exceeds subtitle-safe duration {safe:.3f}s; rewrite before final encode')
...
PY
BASE="$BASE" CONFIG="$CONFIG" FFMPEG="$FFMPEG" python3 - <<'PY'
import json, os, subprocess
from pathlib import Path
base=Path(os.environ['BASE']); data=json.load(open(os.environ['CONFIG'],encoding='utf-8')); ffmpeg=os.environ['FFMPEG']
segments=[]
for i, f in enumerate(data['frames'], 1):
    idx=f'{i:02d}'
    seg_file=f'seg-{idx}.mp4'
    src=base/'source'/'normalized'/f['file']
    out=base/'tmp'/seg_file
    duration=float(f['duration'])
    dx=float(f.get('panX',0)); dy=float(f.get('panY',0))
    fade_start=max(0.0, duration-0.18)
    vf=(
        "scale=1180:2098:force_original_aspect_ratio=increase,"
        "crop=1180:2098,"
        f"crop=1080:1920:x='(iw-ow)/2+({dx}*t/{duration:.2f})':y='(ih-oh)/2+({dy}*t/{duration:.2f})',"
        "setsar=1,"
        f"fade=t=in:st=0:d=0.16,fade=t=out:st={fade_start:.2f}:d=0.18,"
        "fps=30,format=yuv420p"
    )
    subprocess.run([ffmpeg,'-y','-v','error','-loop','1','-t',f'{duration:.2f}','-i',str(src),'-vf',vf,'-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p',str(out)],check=True)
    segments.append(f"file '{seg_file}'")
(base/'tmp'/'segments.txt').write_text('\n'.join(segments)+'\n',encoding='utf-8')
PY
...
BASE="$BASE" VIDEO="$VIDEO" FFMPEG="$FFMPEG" python3 - <<'PY'
import os, subprocess
from pathlib import Path
from PIL import Image
base=Path(os.environ['BASE']); video=Path(os.environ['VIDEO']); ffmpeg=os.environ['FFMPEG']; times=[2,10,20,32,45,57]; imgs=[]
for t in times:
    out=base/'qa'/f'frame-{int(t):04d}.png'
    subprocess.run([ffmpeg,'-y','-v','error','-ss',str(t),'-i',str(video),'-frames:v','1',str(out)],check=True)
    imgs.append(out)
