...
- Active-title hourly chapter output is now 6,000-6,500 Traditional Chinese non-whitespace characters for new or still-unpublished chapter work unless the user explicitly requests a shorter format. Previously published chapters may remain at their already-published length and must not be padded retroactively.
...
- Hourly automation must save generated chapters into the matching active title's canonical chapter location, using `src/resource/<novel title>/文章/` for migrated folders and the title root for unmigrated folders, then update `src/resource/manifest.json` so the static GitHub Pages site can load the newest content. Archived inactive titles under `src/resource/backup/` must not be touched during routine hourly generation.
- Future routine updates and hourly chapter generation should target only the novel text for the 5 active category leaders above unless the user explicitly broadens scope. The other 35 titles stay frozen in `src/resource/backup/` for now.
...
- Automation cadence target: every hour on the hour in Asia/Taipei, update only the 5 active category leaders with one new 6,000-6,500 Traditional Chinese non-whitespace character chapter each where feasible.
...
- Automation publish requirement: after successful validation, create a Git commit such as `chore: hourly AI novel update YYYY-MM-DD-HH` and push to the configured GitHub upstream. If git status is dirty at start, record the baseline and avoid committing unrelated pre-existing changes. If commit or push fails because remote/auth/upstream is unavailable, report the failure clearly.
...
- 2026-06-08 hourly-automation revision: routine novel generation now runs every hour on the hour, not once daily. Active titles should continue from each title's current maximum chapter number rather than assuming all five are synchronized forever.
...
- 2026-06-08 archive revision: inactive titles are temporarily archived under `src/resource/backup/`. They stay in the library but are outside routine hourly generation scope.
